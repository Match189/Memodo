...
- 🖥️ **桌面组件**（Windows）—— 常驻桌面的便签墙 / 任务列表：勾选、钉住、拖动，不占任务栏；Win+D 显示桌面也不受影响。
...
桌面组件（Windows）—— 钉板视图 + 自定义艾森豪威尔四象限背景，壁纸随仓库提供：
...
| 领域 | 内容 |
|---|---|
| 📌 图钉板 | Windows 桌面组件钉板视图（自由布局，拖动/缩放/图钉色）· Android 图钉板 Tab（自适应网格） |
...
> Windows 端的钉板体验由**桌面组件**的钉板视图承担（自由布局、自动跟随数据）；Android 端则为应用内「图钉板」Tab。

- **主窗口** —— 左侧切换 待办 / 备忘 / 设置。顶部输入框 + 回车（或"添加"）新建；勾选完成（划线并归入"已完成"分组）；✎ 编辑 · 🗑 删除。
- **备忘** —— 👁 眼睛图标切换该条是否显示在钉板（桌面组件便签墙）。
- **桌面组件** —— 钉板视图：拖动摆放（位置自动保存）、双击空白处快速添加、双击便签编辑、圆圈勾选完成、待办/备忘切换视图。托盘图标：左键打开主窗口；右键菜单含 显示组件、新建待办/备忘、立即同步、设置、开机自启、退出。
- **备份** —— 设置 → 导出/导入 JSON 全量快照。

**Android**

- 底部 Tab：待办 / 备忘 / 图钉板 / 设置。
- **待办** —— 右滑：完成或撤销 · 左滑：删除（需确认） · 顶部输入框新建。
- **备忘** —— 右滑：切换是否上钉板（同 👁 图标）。
- **桌面小组件** —— 勾选框即时完成 · 点行打开应用 · 待办/备忘 Tab 切换。
- **系统分享转备忘** —— 任意应用里分享文字给 Memodo 即存为备忘。
- **备份** —— 设置 → 导出/导入 JSON。
