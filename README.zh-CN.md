...
> 两个长得像但不同的界面：**图钉板**是主窗口里的页面，手动钉卡片；**桌面组件**是独立的常驻桌面窗口，自动显示所有未完成待办与可见备忘。

- **主窗口** —— 左侧切换 待办 / 备忘 / 设置。顶部输入框 + 回车（或"添加"）新建；勾选完成（划线并归入"已完成"分组）；✎ 编辑 · 🗑 删除。
- **备忘** —— 👁 眼睛图标切换该条是否显示在钉板。
- **图钉板** —— 顶部工具栏：「钉」把已有待办/备忘钉上板 ·「新建」建便签卡 · 拖动摆放 · 双击卡片编辑 · 滚轮缩放 · 1:1 复位 · 网格吸附开关。
- **桌面组件** —— 双击空白处：快速添加 · 双击便签：编辑 · 圆圈：勾选完成 · Tasks/Memos 切换视图。托盘图标：左键打开主窗口；右键菜单含 显示组件、新建待办/备忘、立即同步、设置、开机自启、退出。
