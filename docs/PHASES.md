...
# 2026-08-29 关闭"打开几秒自动关闭" bug

## 现象

启动应用后 15s 左右进程自动消失，stderr 无异常。PowerShell 隐
藏窗口模式无法复现，但正常窗口模式稳定复现。

## 根因

`desktop_multi_window 0.2.x` 在预创建子窗口（隐藏在任务栏后）时
是**真子进程**（不是 Engine 复用），约 15s 后子进程被主进程 dispose
并退出。PowerShell 隐窗口模式立即退出，正常模式延迟 15s。

## 修复

- `preCreate` 默认值改 `false`（文档已说明 desktop_multi_window 0.2.x
  此功能有风险，需要秒开可手动启用）
- 如要"秒开"，用户主动启用后偶发进程退出，重启即恢复
- 这是 desktop_multi_window 0.2.x 的固有限制，没有真正不破坏主进程的预创建

## 验证

t=60s memodo 仍然存活（主+任务栏图标，标题"念念 Memodo"）
